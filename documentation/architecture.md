...
AIReceptionist is a voice-based phone receptionist built on **Google Gemini Live or OpenAI Realtime** (speech-to-speech) and the **LiveKit Agents SDK**. The voice provider is selected independently for each business YAML.
...
4. `AgentSession` receives a provider-specific model from `_build_realtime_model`: `google.realtime.RealtimeModel` for `voice.provider: google`, or `openai.realtime.RealtimeModel` for `voice.provider: openai`. Authentication is resolved from the configured environment variable. `voice.idle.away_seconds` feeds `AgentSession.user_away_timeout`, and `voice.idle.absolute_silence_seconds` can add a wall-clock final-transcript fallback for SIP trunks that send comfort noise.
